...
### 10.6 普通键盘与全局键盘统一 UI 时序（2026-08-21）

2026-09-22候选更正：下述历史“按下/Starting即正在听”会误导用户。当前候选按下及Starting显示“正在连接语音，请按住稍候”，只有Listening才显示“正在听，请继续说”。Starting松手必须取消连接并明确提示未就绪，不可延后开麦；空结果必须发出终止UI的错误回调，不得只发Idle而留下校准状态。该候选不缓存或提前采集音频，完整端到端验收仍待完成。
...
- 全局键盘的候选/语音栏根视图必须始终保留固定尺寸。空闲态只能隐藏内部控件，不能通过切换根视图 `VISIBLE / INVISIBLE` 改变水族 Surface 尺寸，否则按下语音时会出现画面跳动或缩放。
- 覆盖在水族层上方的语音按钮可以用不消费事件的观察器把 `DOWN / MOVE / UP` 镜像给鱼群和涟漪，但 ASR 手势处理器仍是录音生命周期的唯一所有者。
