...
- `f4da0687` Release 的 D0 实测内部 surface 为 1440×471，10 条鱼连续 29.3–29.9 FPS，未发生 EGL/GL 或应用崩溃。后续 `ccbb58d7` Release 保持同一 30Hz、网格和水波路径，但将 CPU 恒速平移与 GPU 独立尾摆改为共享 `swimPhase`：每个尾鳍半拍产生推进力，水阻消耗速度，转向受划水力度限制，GPU 同步呈现该次划水。该版本已覆盖安装到 `.63`，按用户要求没有启动或交互测试，目标 Mali-G52 的 Shader 运行时状态和主观游动效果待现场确认。
