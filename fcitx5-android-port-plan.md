...
- `f4da0687` Release 的 D0 实测内部 surface 为 1440×471，10 条鱼连续 29.3–29.9 FPS，未发生 EGL/GL 或应用崩溃。`ccbb58d7` 首次把 CPU 与 GPU 改为共享 `swimPhase`，但错误地同时用划水强度限制推力和位移，现场表现近似僵住。修正后的 `c8c710b0` 只让尾摆产生加速度，由水阻积分成连续速度，位置只积分一次；并增加鱼头反相摆动、后半身纵深波和随速度变化的尾鳍 Z 轴动作。该 Release 已覆盖安装到 `.63`，按用户要求没有启动或交互测试，目标 Mali-G52 的 Shader 运行时状态和主观游动效果待现场确认。
