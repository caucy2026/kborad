...
- `/Volumes/ORICO/kemi/kboard` is the single canonical Git repository.
- `fcitx5-android/` is a normal tracked source directory, not an independently maintained repository.
- Use only the root `main` branch for source, documentation, and release artifacts. Do not create backup or release branches.
- Run Git commands from the repository root and stage task files explicitly. Do not include generated build output or `lib/fcitx5/src/main/cpp/prebuilt`.
