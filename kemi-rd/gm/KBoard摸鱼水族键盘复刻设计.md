...
> 目标效果：全局键盘下方是一整块沉浸式池塘；每次从普通键盘切入时，10 条鱼依当天是周一到周日游成数字 1–7，短暂保持并游散后随机进入巡游、跟随和两鱼戏耍；金鱼依靠尾鳍和胸鳍真实游动；投喂时鱼群分为抢食、顺游、逆游和谨慎四种角色，松手轮换三种散场；空闲时会跟随、追逐、并游和交叉嬉戏。触摸驱动鱼群并在按下时播放一次水滴声，但不再生成视觉涟漪；持续渲染稳定在 30Hz。最终版本不含水草。
...
| `fcitx5-android/app/src/main/java/org/fcitx/fcitx5/android/input/keyboard/DesktopKeyboard.kt` | 水族层和原生按键层组合、触摸观察、单次水滴声、底部水域、组合键提示和桌面功能键 | `afd627a5557c138add3e24b71b76fc000b4c94f1dd676fa55ec98220a5639b57` |
...
4. `DOWN` 更新鱼群目标并播放一次水滴声但不创建视觉涟漪；`MOVE` 只跟手；`UP/CANCEL` 启动差异化散场且无第二声。

## 2. 视觉和交互验收定义

空闲时 10 条鱼应有不同颜色、花纹、深度、体型和速度性格，覆盖键盘全高，包括最下面的功能键水域。不能形成等距队列，也不能全部同步摆尾。空闲期间每隔随机时间只有一条鱼出现短暂的头部、背部和尾膜流动闪光，不能所有鱼同时发光或常亮。

触摸瞬间所有鱼开始转向触点，但背对目标的鱼先卷尾、张胸鳍制动和快速转身，不能沿旧方向继续滑远。水面不显示触点动画，避免每次输入增加 GPU 片元计算和 uniform 上传；按下仍播放一次低音量真实触水声，释放没有第二声。
...
松手后鱼保留当前惯性，再在约 3.4 秒内轮换执行放射散开、双群反向分流或螺旋扩散，之后按个体状态恢复独立路线、同群跟随或双鱼绕游。
...
DONE    用随机 offset 保证 ROUTE / FOLLOW / PLAY 同时存在，各鱼独立持续 3.8–10.0s，并恢复随机单鱼闪光
```
...
5. 投喂场景在 3 组变体间轮换，随机选一条幸运鱼。触摸不创建水面动画，但播放一个预加载水滴样本。
...
关闭吸引，清除幸运鱼触摸闪光，开启 3.4 秒散开。散场模式按触摸变体轮换：
...
V1.45 已完整删除水草 program、网格、uniform、Draw Call、`touchPlant` 状态和草根碰撞。绘制顺序固定为 `水面 -> 鱼 -> Android 键帽`。不得只隐藏水草图像而保留旧碰撞或 Shader；复刻最终效果时不要移植 V1.40/V1.42 的植物实验代码。
...
声音来自 BigSoundBank `Drops of water #1` 的 4 个 CC0 现场录音切片，由 `prepare-aquarium-water-touch.py` 可重复生成。用 `SoundPool(maxStreams=2)` 在进入全局模式前异步预加载，默认音量系数 0.30；每次 `DOWN` 停掉上一条未结束的尾音，再轮转播放下一样本。`MOVE` 和 `UP` 不播放，避免一次按键出现按下/释放双音效。普通键盘已有的系统声音/触觉设置保持原样。
...
8. 确认触摸路径不创建 ripple 状态、不上传触点 uniform；复制四个 WAV、SoundPool 预加载和 DOWN 单次播放策略。
...
- DOWN 让鱼群目标跟手且只有一次水滴声，水面无视觉涟漪；MOVE 无重复声，UP 正常散开且无第二声。
...
- 触摸首帧可见浅凹陷，不是延迟出现的圆圈。
- 触点可见中心暗凹、偏置亮脊、外扩前导波和后随波峰/波谷；形状轻微非圆，约 2.25 秒衰减，不扭曲整块键盘。
- 声音自然、轻、每按一次只有一次。
...
| 金鱼/涟漪全部消失 | 任一附加 Shader program 链接失败使整个引擎停止 | 删除非必要水草 program；所有新增 Shader 必须在目标 Mali 真机链接验证 |
| 大小鱼仍互相穿模 | 所有鱼沿用固定分离距离 | 按 renderedScale 计算双方碰撞半径并求和 |
| 十条鱼全叠在触点 | 所有鱼追同一个坐标 | 抢食/顺游/逆游/谨慎四角色使用不同半径与角运动，叠加个体碰撞半径 |
...
| 连续输入时 GPU 负载升高 | 恢复了历史 ripple uniform 或片元波列 | 删除触点水面状态、uniform 上传和视觉波列；保留预加载、单 stream 的水滴声 |
...
versionName 529adb53
versionCode 112
ABI         arm64-v8a
APK         fcitx5-android/build/kboard.apk
SHA-256     35008f75e71616782fa8c657f752b502cd942444f4e6a4bd4d3c2430b86d07d7
签名        v1=true, v2=true
证书 SHA-256 c8a2e9bccf597c2fb6dc66bee293fc13f2fc47ec77bc6b2b0d52c11f51192ab8
```

该 Release 已完成 Kotlin、Lint Vital、R8、arm64 原生组件和 v1/v2 签名构建验证，并覆盖安装到 `192.168.3.63`。Android 12/Mali-G52 真机从普通键盘切入全局键盘实测星期一 `digit=1` 完整经过 FORM/HOLD/DEPART/DONE，结束活动为 `route:4,follow:3,play:3`；1080×451、10 条鱼保持 28.8–29.9 FPS。H/J 键受控触摸截图可见凹陷、亮脊和衰减波列，日志出现 AudioTrack 创建，且无 EGL/GLSL/renderer stopped/应用崩溃。水滴听感仍以现场扬声器人工验收为准。
...
V1.49 当前无视觉涟漪基线：触摸仍驱动鱼群 C-start、跟手、投喂角色和散开，但删除 `Ripple`/循环槽、`uRipples`、每帧 uniform 上传和片元波列/折射计算。保留单 `uTime` 的低成本水面流光、按下单次水滴声、按键触觉、语音和普通键盘功能。其他项目应以本文顶部列出的当前文件哈希复刻。

V1.49 正式包为 `80325aee/122`，SHA-256 `f658a757aeb09768c687d1e5be12782b8524d1f53d84009cfa6634a3b58f3003`。签名 Release 构建及 APK 内 4 个 WAV 资源检查通过，并已无损覆盖安装到 63；视觉涟漪从源码执行路径移除，水滴声音仍需以设备扬声器现场听感为准。
