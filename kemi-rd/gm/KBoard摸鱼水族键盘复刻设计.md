...
> 目标效果：全局键盘下方是一整块沉浸式池塘；金鱼依靠尾鳍和胸鳍真实游动，触摸后长出摆动水草并争先恐后游向附近，松手后部分围草游、部分散开；空闲时会跟随、追逐、并游和交叉嬉戏；触点产生轻微非圆涟漪和一次真实水滴声；持续渲染稳定在 30Hz。
...
| 文件 | 用途 | SHA-256（`dd5d2700`） |
|---|---|---|
| `fcitx5-android/app/src/main/java/org/fcitx/fcitx5/android/input/keyboard/aquarium/DesktopAquariumView.kt` | EGL、30Hz 渲染线程、水面/水草/鱼体 Shader、个体速度、随机闪光、按尺寸碰撞、鱼草互动和水动力 | `cd4eb00d26df2effd65d79582e200042eb5bb53a3f4a63551c6acceedfb9fbe5` |
...
4. 触摸只在 `DOWN` 新建一簇水草、涟漪并播放一次声音；`MOVE` 只更新目标，`UP` 让部分鱼围草、部分鱼散开。
...
2. 在 4 个水草循环槽中写入一簇新水草，根部 X 限制 `[-0.88,0.88]`、Y 限制 `[-0.93,0.70]`。
...
关闭吸引，开启 3.4 秒散开。最新水草的 9.5 秒兴趣期内，约一半鱼优先围草游，另一半以自身 seed、索引和 `±0.24rad` 随机扰动散开到半径 `0.44..0.86`；底层鱼的普通散开目标 Y 仍不高于 `-0.66`。之后行为状态推进 1–3 步，并在散开结束后的 4–10 秒维持新状态。

### 水草网格、气泡和鱼草槽位

水草必须使用独立小网格 Pass，绘制顺序为 `水面 -> 水草 -> 鱼 -> Android 键帽`，不得在全屏水面片元 Shader 中重复计算植物 SDF：

- 每簇 7 片叶，每片 5 段，每段 2 个三角形；另有 3 个气泡 quad，共 228 顶点；
- 生长 `0.52s`，寿命 `22..32s`，最后 `3.2s` 淡出；最多保留 4 簇；
...
```text
angle = index×2.3999632 + seed×0.23
      + plantAge×direction×(0.34 + fract(seed)×0.18)
radius = 0.115 + collisionRadius×1.15 + (index mod 3)×0.035
targetX = plantX + cos(angle)×radius
targetY = plantY + 0.055 + sin(angle)×radius×0.72
```

水草根部是柔性障碍：安全距离为 `fishCollisionRadius + 0.055`，进入后增加 0.78 倍分离力，若草根位于鱼正前方还要增加制动。水草和槽位只能改变导航意图，不能直接位移鱼。
...
- DOWN 有一簇新水草、一次涟漪和一次声音，MOVE 无新水草/声音，UP 无第二声。
- 手指滑到语音键和底部空白水域，鱼也持续跟随。
- 松手后约一半鱼围最新水草分槽游，另一半在 3.4 秒内明显散开。
- 连续点击第 5 个位置只替换最老水草，屏幕最多 4 簇；旧草 22–32 秒后平滑淡出。
...
- 最大鱼与最小鱼具有不同碰撞半径；围草、追逐和交叉时身体及长尾不穿模。
...
| 水草一多 FPS 明显下降 | 在全屏水面片元 Shader 里计算每片叶 | 独立 228 顶点小网格，每簇一次 Draw Call，最多 4 簇 |
| 大小鱼仍互相穿模 | 所有鱼沿用固定分离距离 | 按 renderedScale 计算双方碰撞半径并求和 |
| 十条鱼全叠在草根 | 所有鱼追同一植物坐标 | 一半访问、一半散开；访问鱼按黄金角和碰撞半径分槽 |
