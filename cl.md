...
- 63 配套 PAD 是 `1.4.126+309`，PAD APK SHA-256 `53ccdfacf08107bcf0c61ac731fd476ec5683811ff81a4387229540bb7986f4d`。这只确认安装组合身份和启动条件，不代表极简语音、全部组合键或跨屏键盘完整验收通过。
- 云端 `main` 已有 `1.4.5+222` 的鉴权稳定性源码（`e8909d95`）；当前 63/75 实装仍为上述 `1.4.4+212` 候选。不能把云端更新源码误报为已安装或已发布。旧 404a detached 工作树仍有大量历史未提交改动，不作为本次云端主线覆盖来源。


## 2026-09-23 - 1.4.5 正式版本源码

- 合入鉴权短超时、最多重试一次、不复用闲置 HTTP 连接，以及取消/过期会话不迟开麦的保护；同时保留远端极简语音与布局改动。版本名升至 1.4.5，arm64-v8a 版本码升至 222。
- 16.24 曾抓到两次新 TCP 连接已送达鉴权服务器却未在 2.5 秒内收到 HTTP 响应；客户端修复不能保证服务端或网络不再发生此现象。真实语音识别与长期稳定性仍待现场验证。

## 2026-09-23 - 语音启动可靠性修复候选，交16.24现场验证

- 已确认此前偶发慢启动发生在 HTTP 鉴权完成之前；DNS/TCP/连接复用/服务端的具体责任仍待阶段日志确认，不宣称唯一根因已查明。
- 参考桌面助手分离 HTTP/WS 与后台完整读取响应的模式，新增 `AsrAuthRequest.kt`，调整 `IflytekAsrClient.kt`：单次鉴权2500ms完整调用期限、临时失败最多一次重试、8s主线程启动保护、会话回调原子归属校验、麦克风及连接异常终止、脱敏耗时日志。鉴权拒绝、安全策略和权限错误不绕过；Starting松手仍取消，不允许迟开麦。未修改桌面助手或键盘布局。
- 新增 `AsrAuthRequestTest.kt` 8项；完整APP 111项单测通过，Release Kotlin检查及本次assembleRelease成功。平台证书SHA-256为 `c8a2e9bccf597c2fb6dc66bee293fc13f2fc47ec77bc6b2b0d52c11f51192ab8`，v1/v2/v3验签通过。
- 用户明确批准用独立WSL目录生成同版本测试候选并覆盖16.24。候选 `bin/KBoard-1.4.4-182-asr-fix-20260923.apk`，SHA-256 `569a042ab2e92acc8e19c9d2e5ff877f2d23fb2a9341b245879a750442a2ae29`；`adb install --no-incremental -r`成功，已安装APK回读哈希一致，系统UID=1000，默认IME不变，未清数据。
- 用户选择现场自行测试，未远程开启麦克风。连续启动、取消重按、闲置后启动、网络恢复、真实识别与收字仍待验；8s主线程定时器也不构成主线程阻塞下的严格实时期限。该包不是正式发布验收通过声明；未推送Git、商城或H730仓库。详见 `iflytek_asr_interface_doc.md` §10.9及本地 `diagnostics/asr-startup-20260923/`。

### 当日后续：闲置鉴权连接造成的间歇性启动等待

- 16.24 用户现场使用前一候选时再次出现数秒无法启动。脱敏阶段日志中多次看到首次鉴权在 `auth_request_start` 后无响应、没有 DNS/TCP 新连接，2500ms超时；重试建立新连接后通常约50ms得到HTTP 200，WebSocket和开麦再约100ms。一次重试的新连接也无响应，提示服务端/网络仍有独立偶发失败。
- 本机模拟服务器保持旧HTTP socket打开但不再响应：默认连接池的第二次鉴权测试失败；`AsrAuthRequest.kt` 将鉴权连接池设为零闲置后，同一测试通过。完整APP 112项单测0失败，Release构建成功。仍逐次执行鉴权，不缓存鉴权结果。
